...
function computeWeeklyRevenue() {
  ...
  weeklyDays.innerHTML = chartDays.map(day => {
    const barWidth = totals[day] === 0
      ? 0
      : (totals[day] / maximumValue) * 100;
...
function details(transactionCode, name, movie, amount, seats, tickets, status) {
...
function completeBooking() {
  ...
  fetch("complete_booking.php", {
    ...
    body: "transaction_code=" + encodeURIComponent(currentTransactionCode)
  })
